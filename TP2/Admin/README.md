...
```console
geoffrey > docker run --memory 1g --cpus 1.0 debian:latest echo 'ratio'
ratio

geoffrey > cat docker-compose.yml 
version: '3'
    ...
    deploy:
      resources:
        limits:
          cpus: '1.0'
          memory: 1G
    entrypoint: echo ratio

geoffrey > docker compose up -d && docker compose logs
[+] Running 1/1
 ✔ Container admin-ratio-1  Started                                                                              0.0s 
admin-ratio-1  | ratio
```
...
```console
geoffrey > docker run --user 1010 -it debian:latest whoami
...
[+] Running 1/1
 ✔ Container admin-ratio-1  Started                                                                              0.0s 
admin-ratio-1  | whoami: cannot find name for user ID 1010
```
